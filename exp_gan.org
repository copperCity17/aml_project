...
#+RESULTS:
: dict_keys(['testA', 'testB', 'trainA', 'trainB'])
...
fig, ax = plt.subplots(2, 2, sharex=True, sharey=True, figsize=(60, 46))
...
for i in range(len(imgs)):
  ...
  if i % 2 == 0:
    ax[row,col].imshow(imgs[i][0] * 0.5 + 0.5)
  else:
    ax[row,col].imshow(imgs[i][0] * 0.5 * contrast + 0.5)


plt.savefig(fig_dir/"test.png", bbox_inches="tight")
fig_dir/"test.png"
#+END_SRC

#+RESULTS:
[[file:/Users/antoniomendes/Desktop/uva_data_science/semester_01/block_02/AML/Kaggle_Project/figures/test.png]]

#+BEGIN_SRC python :results file
sample_input = sample_horse
translation = generator_g(sample_input)

imgs = [sample_input, translation]
...
for i in range(len(imgs)):
  ...
  if i % 2 == 0:
    print("hello")
    ax[i].imshow(imgs[i][0] * 0.5 + 0.5)
  else:
    ax[i].imshow(imgs[i][0] * 0.5 * contrast + 0.5)
